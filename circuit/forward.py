...
class Forward:
    ...
    def compile_params(self):
        ...
        self.vp, self.vin_max = voltage_compiler(vdc_min = self.vin_dc_min, vdc_max = self.vin_dc_max, vac_min = self.vin_ac_min, vac_max = self.vin_ac_max)
        ...
        self.d_max, self.turns_ratio_list[0], self.vro = d_n_vro(topology = "forward", vs = (self.vo_list[0] + self.vf_list[0]), vp = self.vp, d = self.d_max, n = self.turns_ratio_list[0], vro = self.vro)

        self.turns_ratio_list = (np.array(self.vo_list) + np.array(self.vf_list)) / (self.vp * self.d_max)
        # self.turns_ratio_list = np.insert(self.turns_ratio_list, 0, 1)
        if self.mode == "BCM":
            # critical output inductance calculation
            d_min = (self.vo_list[0] + self.vf_list[0]) / (self.turns_ratio_list[0] * self.vin_max)
            self.lo_crit = (self.vo_list[0] + self.vf_list[0]) * (1 - d_min) / (2 * self.io_list[0] * self.fs)
